...
def make_label_dict(labels):
    ...
    for i, label in enumerate(labels):
        l[i] = label
    return l
label_dict = make_label_dict(get_labels('c://test.csv'))
print label_dict    
...
betweenness = nx.eigenvector_centrality(gr)
between = betweenness.items()
between.sort(key=lambda x:x[1],reverse=True)
print between


nx.draw(gr, node_size=3000,labels=label_dict, with_labels=True,node_color='red',edge_color='black',text_font='arial')
plt.show()     
